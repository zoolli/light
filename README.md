...
| G | 電腦 | － | `研華`、`廟`、`研華*3` | 同上 |
| H | 正規日期 | 系統產 | `2026-10-17` | 由 E 欄自動推算，纯粹给排序／篩選用 |
...
|---|---|
| `國10/17前` | `2026-10-17` |
| `國115/02/09前`、`國115-02/09前` | `2026-02-09`（民國 > 1911 才視為西元年） |
| `12/10or12/17` | `2026-12-10`（取第一個日期） |
| `10月15日` | `2026-10-15`（補今年） |
| `已送燈`、`待定`、空 | 留空 |
...
| `SPREADSHEET_ID` | 試算表網址 `/d/` 與 `/edit` 之間那串 | 建議（不填則用綁定的活頁簿） |
...
| LINE Verify 失敗 | 沒發新版本、網址少 `/exec`、權限不是「任何人」 | 重走 4.3 |
| 只有「系統異常」 | 試算表分頁名不對、`SPREADSHEET_ID` 錯、沒授權 | 看執行記錄的堆疊 |
