...
- 測試：`test/light.test.js`（純 Node 跑的 stub 測試，不需 GAS 環境）→ `node test/light.test.js`，目前 94 項全過
...
| E | 送燈日期 | － | `國10/17前`、`12/10or12/17`、`已送燈` | **保留使用者的寫法**，不被系統改寫 |
| F | 軟體 | － | `廟幫手`、`冠緯`、`其他` | 可省略，之後用瀏覽器補 |
| G | 電腦 | － | `研華`、`廟`、`研華*3` | 同上 |
| H | 正規日期 | 系統產 | `2026-10-17` | 由 E 欄自動推算的**國曆**日期，只給排序／篩選用 |
| I | 備註 | － | `分兩批送，第二批國12/01前` | 只有在訊息裡明確寫「備註：…」才會填 |
| J→ | 你的公式欄 | － | `=SUM(D:D)` | 合計、小計請放 **J 欄以後**，系統只看 A~I |
...
- 投影範圍是主表的 `A2:I`（含備註欄）；欄位以後再加，投影會自動跟著長寬走（`maxColumnIndex()`）
...
### 2.2 送燈日期：國曆／民國／農曆 對應表（E 原樣 → H 正規化）

系統**只換算國曆**。E 欄永遠保留你打的字，H 欄是給你排序、樞紐分析用的輔助值。
...
| `農曆10/17前`、`舊曆11/05`、`陰曆12/01前` | **留空（刻意不推）** | GAS 沒有農曆⇄國曆對照表，硬換算一定錯。要查就自己對萬年曆，或把國曆日期補進 E／H |
...
```
用 Node 的 `vm` 载入 `light.gs`，把 `SpreadsheetApp`／`UrlFetchApp`／`CacheService` 等 GAS 服務换成 stub，並塞入真實格式的 17 列種子資料（含合計列、右側公式欄）。涵蓋（共 94 項）：

- 日期正規化（國曆／民國／農曆不換算）十餘種寫法
- 備註欄：明示才寫、可單獨修改、查詢會列出
- 小計列跳過、規格／廟名正規化比對
- 新增：重複攔截、`強制新增`、8 欄落地
