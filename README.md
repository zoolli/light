...
- 測試：`test/light.test.js`（純 Node 跑的 stub 測試，不需 GAS 環境）→ `node test/light.test.js`，目前 143 項全過
...
**新增時遇到「廟名對到既有資料」的處理矩陣**（一律把相關的列列出來給你確認，不會靜悄悄地亂加或亂擋）：

| 命中狀況 | 行為 |
|---|---|
| 廟名完全沒出現過 | ✅ 直接新增，標註「🆕 新廟名（表上是第一筆）」 |
| 同廟、**規格不同** | ✅ 新增，並列出該廟既有規格與燈數（避免你以为加在同一列） |
| 同廟、同規格、**承攬商不同** | ✅ 新增為另一張單，同時提示「第 N 列已有同規格但承攬商為○○」 |
| 同廟、同規格、同承攬商 | ⛔ 擋下，顯示那一列，並用**該列的完整廟名**教你怎麼改（`修改 天成宮(中和) 4*5 OLED 總燈數變成 2500 盞`） |
| 同名廟有多會館且規格也相同（如只打「天成宮」） | ⛔ 先不動資料，列出北投／中和兩列，請你把廟名寫完整再傳；**同時存 10 分鐘補件草稿**，下一句只補全名也接得上 |
| 訊息尾端加 `強制新增` | ✅ 跳過上面所有比對，直接入表 |
...
- **授權**：走 2 或 1 時，函式第一次執行會要求 Drive／Docs 讀取範圍，點了才讀得到；失敗時只會退回下一個來源，不影響登記
- **上下文**：記最近 5 輪（`CHAT_HISTORY_TURNS`），放 CacheService，10 分鐘沒聊就清空
**操作手冊來源**（自動偵測，第一個有條目的生效）：

| 優先序 | 來源 | 設定方式 |
|---|---|---|
| 1 | 單一 Google 文件 | `CHAT_MANUAL_DOC_ID` |
| 2 | Drive 資料夾（内所有文件） | `CHAT_MANUAL_FOLDER_ID`，例 `https://drive.google.com/drive/folders/←這串→` |
| 3 | 同一份試算表的 `操作手冊` 分頁 | A 主題／關鍵字、B 內容、C 範例句，一題一列 |
| 4 | 都没有 | 退回 `/help` 内建文案 |

**格式只支援三種**（`DocumentApp` 讀不到 PDF 與上傳的 .docx 的文字）：

| 你放的檔案 | 能不能讀 | 怎麼辦 |
|---|---|---|
| Google 文件（`.gdoc`） | ✅ | 推薦。從網站產出後貼進一份新的 Google 文件即可 |
| `.txt`、`.md` | ✅ | 直接放進資料夾就會讀；`#` 標題符號與 `**粗體**` 會自動清掉 |
| `.docx`（上傳的 Word 檔） | ❌ | 右鍵 → 開啟方式 → Google 文件 → **檔案 → 另存為 Google 文件**，存成 Docs 後才讀得到 |
| `.pdf` | ❌ | GAS 沒有 PDF 內嵌文字擷取（要抽就得再掛 Vision API）。建議改用 .docx 轉 Docs 或直接貼成 Docs；跳過的檔案會在執行記錄列出檔名 |

> 檔名含「合計」的文件會被整份略過。每次讀取結果快取 10 分鐘（`CacheService` 上限），所以改完手册最多等 10 分鐘生效。

**手册該怎麼寫才切得準**（解析邏輯在 `parseManualText()`）：

| 你的寫法 | 系統怎麼切 |
|---|---|
| Markdown：`## 如何點燈（安奉流程）` 換行接內文 | **以 # 標題為界**，標題＋其後所有內文＝一條（中間空行不會切斷） |
| Google 文件／純文字（沒有 `#`） | 以**空行**為界，段內首行當主題 |
| 條目裡寫 `例：小幫手 …` | 整段進 prompt，模型會照抄這行當可複製範例 |
| 檔名 | 會當前綴（`光明燈點燈手冊｜如何點燈`），方便模型說「見手册某節」 |

三條實務建議：
1. **沒有圖也能講清楚**：用「位置＋文字」取代截圖，例如不寫「如圖」而寫「最右邊那個下拉選單」「第二個分頁」。真的需要版面，就放一行純網址，LINE 會自動變可點連結（我現在只發文字，**不會顯示圖片**；要發圖得走 LINE blob 上傳，另外做）
2. 一個小節一件事、控制在 10 行內：太長會被 `CHAT_MANUAL_MAX_CHARS`（預設 4000 字）截斷，而且關鍵字比對容易選錯
3. 上傳前先在本地驗證切條結果（會印出條目清單，以及某個問句實際挑到哪幾條進 prompt）：
```bash
node test/preview_manual.js 你的手册.md "如何點燈"
```
...
```
用 Node 的 `vm` 载入 `light.gs`，把 `SpreadsheetApp`／`UrlFetchApp`／`CacheService` 等 GAS 服務换成 stub，並塞入真實格式的 17 列種子資料（含合計列、右側公式欄）。涵蓋（共 143 項）：

- 日期正規化（國曆／民國／農曆不換算）十餘種寫法
- 備註欄：明示才寫、可單獨修改、查詢會列出；〔國…〕標記的插入／替換／不堆疊、可關掉
- 小計列跳過、規格／廟名正規化比對
- 新增：同廟比對矩陣（新廟／不同規格／不同承攬商／同承攬商重複／多會館同名先列清單＋存草稿）、`強制新增`、A~H 落地
...
- 操作手冊四來源（單檔 → 資料夾 → 分頁 → `/help`）與優先序、`PDF/.docx 略過並留記錄`
- 手册解析：`#` 標題為界／無標題以空行為界、大標題不成一條、單行 HTML 仍可用、markdown 符號與 base64 圖清除
...
| 議題 | 結論 | 理由 |
|---|---|---|
| 送燈日期自動寫進 Google 日曆 | **不做** | 排程用試算表的日期篩選／樞紐分析就夠了；接日曆會多出 Calendar 授權與「改日期要同步事件」的修補邏輯，代價大於效益 |
| 另外推 LINE 彈出提醒 | **不做** | 同上，提醒靠 LINE 端自行掌握 |
| 每家承攬商一份檔案（真隔離） | 暫不做 | 目前用主表 + 公司分頁唯讀投影；若日後真的要求互不可見，再把 `USER_IDENTS` 擴成存各自的 `SPREADSHEET_ID` |
| 向量資料庫／RAG 檢索手册 | **暫不做** | 手册規模是幾十條、幾千字，現行的「關鍵字重疊挑最相關 5 條」已是極簡檢索且零成本；上 RAG 要多一組金鑰、外部服務與故障點。**升級訊號**：手册超過約 3 萬字、或開始選錯條目 → 再做「土產 RAG」：用 Gemini embedding 把段落向量存進一個 `手册索引` 分頁，檢索時算餘弦相似度取 top-3，仍不需要外部資料庫 |
| 農曆→國曆自動換算 | **不做** | 需要內掛對照表；換算錯比留空更糟。E 欄保留原文、備註不插〔國…〕標記 |
| LINE 端查詢回訊 | 預設關 | 用 `REPLY_MODE=ALL` 可打開；查詢走瀏覽器比較快 |
